...
class DynamicPromptGenerator:
    ...
    async def generate_task_prompt(
        ...
        # 설명용 브리프: 폼 정보 제외
        normalized_instructions = (task_instructions or "").strip()
        requires_downloadable_url = bool(normalized_instructions and "파일 생성" in normalized_instructions)
        simple_request_keywords = ["간단히", "간단하게", "대충", "해줘", "만들어줘", "줘", "make", "create", "please"]
        requires_concretization = False
        if normalized_instructions:
            lowered = normalized_instructions.lower()
            keyword_hit = any(kw in normalized_instructions for kw in simple_request_keywords) or any(
                kw in lowered for kw in ["ppt", "pptx", "brief", "summary"]
            )
            short_request = len(normalized_instructions) <= 60 and normalized_instructions.count("\n") == 0
            imperative_only = normalized_instructions.endswith("해줘") or normalized_instructions.endswith("줘") or normalized_instructions.endswith("주세요")
            requires_concretization = keyword_hit and (short_request or imperative_only)
        ...
        desc_brief = self._build_description_prompt(
            ...
            requires_downloadable_url=requires_downloadable_url,
            requires_concretization=requires_concretization,
        )
    ...
    def _build_description_prompt(
        ...
        requires_downloadable_url: bool = False,
        requires_concretization: bool = False,
    ) -> str:
...
{("""\n**파일 생성 작업 필수 조건:**\n- 작업 지시사항에 '파일 생성'이 포함된 경우, 반드시 다운로드 가능한 URL을 제공할 수 있는 도구나 스킬을 활용하여 결과물을 제공\n- 해당 도구/스킬이 없다면 먼저 적합한 도구 사용 가능 여부를 확인하고, 사용 불가 시 명확한 사유와 함께 실패 상태로 보고\n- 생성된 파일은 사용자나 후속 시스템이 즉시 접근 가능한 형태로 제공되어야 하며, URL의 유효성 및 접근 권한을 다시 검증\n""" if requires_downloadable_url else "")}
{("""\n**단순 지시 구체화 원칙:**\n- 간단한 요청 문장 형태의 작업지시라도 실제 수행 가능한 단계로 세분화하여 시스템 메시지를 작성\n- 파일 생성, 실행, 업로드, URL 제공 등 필수 절차를 명확히 정의하고 사용할 도구와 스킬을 지정\n- Supabase 및 기타 스토리지/전송 도구를 활용해 산출물을 업로드하고, 검증된 다운로드 URL을 제공할 수 있도록 단계별 지침 포함\n- 단순 지시를 그대로 재진술하지 말고, 실행 가능한 상세 액션 플랜과 검증 절차를 추가\n""" if requires_concretization else "")}
...
- 모든 가용 도구를 적극 활용하여 정보 수집:
  ...
  * 파일 생성 작업이면, 'upload_file' 같은 도구를 사용하여 파일을 업로드하여 다운로드 가능한 URL을 폼 내역 최상단에 반영하여 제공
  * URL은 반드시 실제 접근 가능한 URL이어야 하며, href 속성을 포함하여 제공
...
"""
    ...
    def _build_expected_output_prompt(
        ...
        sections = (
            ...
            "- '수행한_작업'은 가독성 좋은 자연어 한 문자열(불릿/번호 허용)으로 작성\n"
            "- 작업지시사항에서 추출한 모든 원자 작업을 누락 없이 서술(누락 시 FAILED 간주)\n"
            "- 다중 데이터 모드: is_multidata_mode=\"true\" 필드는 반드시 배열 형태로 반환(HTML의 실제 name 사용)\n"
        )

        if requires_downloadable_url:
            sections += "- 파일 생성 작업인 경우, 폼_데이터에 다운로드 가능한 URL 필드를 반드시 포함하고, 링크가 실제 접근 가능한지 검증 후 반영 (예: download_url, file_download_url 등 명확한 키 사용)\n\n"
        else:
            sections += "\n"
