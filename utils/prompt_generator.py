...
{f'>> 중요: 이전 작업지시사항대로 했지만 결과가 만족스럽지 않아서 나온 수정 요구사항입니다. 기존 방식을 버리고 이 피드백대로 수정해야 합니다!' if has_feedback else ''}
...
**작업 처리 우선순위:**
{f'1. 피드백 최우선: 작업지시사항을 피드백 내용에 맞게 수정하여 수행' if has_feedback else '1. 작업지시사항 그대로 수행'}{f'''
2. 학습된 경험 활용: 작업지시사항은 그대로 하되, 경험을 참고해서 더 디테일하게 완벽하게 처리''' if has_learned_knowledge else ''}

**핵심 실행 원칙:**
- 작업 범위: 작업지시사항에 명시된 작업만 수행. 추가/후속 작업 절대 금지
- 도구 활용: mem0/memento 정보는 참고용으로만 활용. 실제 작업은 다른 도구들(데이터베이스, API 등)을 사용하여 수행
- 보안/모호성 대응: 보안에 민감한 작업, DB에 저장/수정/삭제 작업, 정보가 모호한 경우, 반드시 human_asked 도구를 사용하여 사용자 승인을 받은 뒤 수행
- 이전 결과물 완전 활용: 이전 작업 결과에 담긴 모든 정보들을 빠뜨리지 말고 활용하여 작업할 것
- 실패 시 대응: 작업 중 오류나 실패가 발생하면 넘어가지 말고, 문제를 분석하고 해결하거나 다른 방법으로 재시도하여 반드시 완료할 것
- 정보 저장/수정: 반드시 도구나 전달된 정보를 최대한 활용해서, 올바른 값들을 찾아서, 누락없이 저장 및 수정할 것

**달성 목표:**
- 구체적으로 무엇을 완성해야 하는지 (작업지시사항 범위 내에서만)
- 어떤 품질과 기준을 만족해야 하는지
- 특별히 주의해야 할 요구사항이 있는지
- 명시되지 않은 연관 작업이나 후속 작업은 절대 수행하지 말 것
- 명시된 작업을 정확하게 수행하기 위해 도구들을 적극 활용하고, 모든 정보를 활용하여 작업할 것

**성공 기준:**
- 명시된 목표들이 전부 달성되어야 함 (일부만 달성하면 실패)
- 요구된 형식으로 결과 제공{f'''
- 피드백 반영 필수: 이전 방식으로 하면 안 되고, 반드시 피드백 내용대로 수정해서 처리해야 함''' if has_feedback else ''}{f'''
- 경험 활용 권장: 학습된 경험을 참고해서 작업지시사항을 더 완벽하게 수행''' if has_learned_knowledge else ''}
...
- 보안/모호성 대응: 보안에 민감한 작업, DB에 저장/수정/삭제 작업, 정보가 모호한 경우, 반드시 human_asked 도구를 사용하여 사용자 승인을 받은 뒤 수행할 것을 명시
